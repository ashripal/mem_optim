...
- cache hit -> compute NOT called, compute_latency_s == 0
- rss fields exist and are numeric
- tokens_per_second computation behavior
- metadata passthrough (device/dtype/truncated/input_tokens/output_tokens)
...
def test_evaluator_cache_miss_then_hit_behavior():
    ...
    assert isinstance(r1["compute_latency_s"], float)
    assert r1["compute_latency_s"] > 0.0

    # Second pass: hit -> compute NOT called
    r2 = evaluate_example(example=example, cache=cache, compute=compute, cfg=cfg)
    assert r2["ok"] is True
    assert r2["cache_hit"] is True
    assert compute.calls == 1  # unchanged
    assert r2["compute_latency_s"] == 0.0
    assert isinstance(r2["latency_s"], float)
    assert r2["latency_s"] >= 0.0

    # Tokens/sec should be None on cache hit (since compute_latency_s=0)
    assert r2["tokens_per_second"] is None


def test_evaluator_tokens_per_second_computed_on_miss(monkeypatch):
    """
    Ensure tokens_per_second is computed as output_tokens / compute_latency_s when
    compute_latency_s > 0. We monkeypatch time.time to make timing deterministic.
    """
    cfg = _Cfg(max_input_tokens=64, max_new_tokens=8)
    cache = LRUCache(capacity=8)
    compute = FakeComputeEngine()
    ...
    # Call order in evaluate_example() on cache miss:
    #   1) total_t0
    #   2) lookup_t0
    #   3) after lookup
    #   4) compute_t0
    ...
    times = iter([100.0, 100.1, 100.2, 100.3, 101.3, 101.5])
    ...
    assert r["compute_latency_s"] == pytest.approx(1.0)
...
def test_evaluator_includes_optional_cuda_fields():
    cfg = _Cfg(max_input_tokens=64, max_new_tokens=8)
    cache = LRUCache(capacity=8)
    compute = FakeComputeEngine()
    ...
    assert "cuda_device_name" in r
    assert "gpu_mem_allocated_mb" in r
    assert "gpu_mem_reserved_mb" in r